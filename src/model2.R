...
#Data Manipulation to get Experiment 6 Data
test <- big.dfs[[6]]
df <- tibble(
...
names(df) <- c('time', paste(rep(c('trend.', 'resid.'), times = 8),
                             rep(vars, each = 2), sep=''))
df$perm_cond_low_us <- data$perm_cond_low_us[data$experiment == 6]
df <- df %>%
  select(-time)


#Start of Adaptive Lasso
...
for(s in s.list){
  ...
  par(mfrow=c(1,1))
  plot(data$time[data$experiment == 6], as.numeric(df$perm_cond_low_us), type = "l", xlab = "", ylab = "")
  lines(data$time[data$experiment == 6],  predict(mod_adaptive, newx = x, s = mod_adaptive$lambda.1se), col=2)
}

#Start of Adaptive Lasso in Experiment 5
test <- big.dfs[[5]]
df <- tibble(
  time = test$water_perm_coef$time,
  trend.water_perm_coef = test$water_perm_coef$trend,
...
for(s in s.list){
  ...
  par(mfrow=c(1,1))
  plot(data$time[data$experiment == 5], as.numeric(df$perm_cond_low_us), type = "l", xlab = "", ylab = "")
  lines(data$time[data$experiment == 5],  predict(mod_adaptive, newx = x, s = mod_adaptive$lambda.1se), col=2)
}
