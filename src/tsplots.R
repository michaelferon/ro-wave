...
#Plots if permeate conductivity
for (i in 1:N) {
...
for (i in 1:N) {
  ...
  if (OUTPUT) {
    pdf(file = paste('../plots/ts/permeate_flowrate/permeate_flowrate', i,
                     '.pdf', sep = ''), height = 4.0, width = 8.67)
    print(g + ylim(min(data$perm_flow_lm), 2.02))
    dev.off()
...
for (i in 1:N) {
  ...
  if (OUTPUT) {
    pdf(file = paste('../plots/ts/reject_flowrate/reject_flowrate', i,
                     '.pdf', sep = ''), height = 4.0, width = 8.67)
    print(g + ylim(1, max(data$rej_flow_lm)))
    dev.off()
...
for (i in 1:N) {
  ...
  if (OUTPUT) {
    pdf(file = paste('../plots/ts/feed_pump_power/feed_pump_power', i,
                     '.pdf', sep = ''), height = 4.0, width = 8.67)
    print(g + ylim(20, max(data$feed_pump_pow)))
    dev.off()
